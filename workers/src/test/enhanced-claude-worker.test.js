...
jest.mock('../../../monitoring/monitoring-setup.js', () => ({
  monitoringSystem: {
...
describe('EnhancedClaudeWorker', () => {
  ...
  describe('Enhanced Marketing Command Parsing', () => {
    test('should parse simple marketing command with workflow context', async () => {
      ...
      mockClaudeService.parseMarketingCommand.mockResolvedValue(expectedResponse);

      // Act
      const result = await worker.parseMarketingCommandEnhanced(input, lhContext);
    ...
    test('should handle complex multi-channel campaign requests', async () => {
      // Arrange
      const input = {
        command: 'Create urgent multi-channel campaign for premium customers with email and SMS, include 20% discount, send tomorrow at 2pm',
        priority: 'high'
      };
      ...
      expect(result.workflowEnhancements.workflowContinuation.requiresHumanApproval).toBe(true);
    });
    ...
    test('should utilize response caching for identical requests', async () => {
      ...
      // Act - First call
      const result1 = await worker.parseMarketingCommandEnhanced(input, lhContext);
      
      // Act - Second identical call (should use cache)
      const result2 = await worker.parseMarketingCommandEnhanced(input, lhContext);
    ...
    test('should handle validation errors gracefully', async () => {
      ...
      // Act
      const result = await worker.parseMarketingCommandEnhanced(invalidInput, lhContext);
  ...
  describe('Saga Pattern Integration', () => {
    test('should generate campaign content with saga compensation data', async () => {
      ...
      worker._generateContentWithSagaPattern = jest.fn().mockResolvedValue(mockContentResult);

      // Act
      const result = await worker.generateCampaignContentSaga(campaignRequest, sagaContext);
      ...
      expect(result.sagaMetadata.compensationData.contentIds).toEqual(['content-1', 'content-2']);
    ...
    test('should handle saga step failure with compensation metadata', async () => {
      ...
      worker._generateContentWithSagaPattern = jest.fn().mockRejectedValue(
      ...
      expect(result.sagaMetadata.stepName).toBe('content-generation');
      expect(result.sagaMetadata.failureReason).toBe('Content generation failed');
    ...
    test('should execute compensation successfully', async () => {
      ...
      // Act
      const result = await worker.compensateContentGeneration(compensationData, sagaContext);
      ...
      expect(worker.sagaCompensations.has('saga-compensation-test')).toBe(false);
    });
  });
  ...
  describe('Cross-Workflow Coordination', () => {
    test('should coordinate state synchronization between workflows', async () => {
      ...
      // Act
      const result = await worker.coordinateCrossWorkflow(coordinationRequest, lhContext);
    ...
    test('should handle event propagation to target workflow', async () => {
      ...
      const coordinationRequest = {
        ...
        event: {
          ...
          data: { campaignId: 'CAMP-001', metrics: { sent: 1000, opened: 250 } }
        }
      };
      ...
      // Act
      const result = await worker.coordinateCrossWorkflow(coordinationRequest, {});
    ...
    test('should handle coordination failures gracefully', async () => {
      ...
      worker._handoffResourceToWorkflow = jest.fn().mockRejectedValue(
        new Error('Target workflow unreachable')
      );
  ...
  describe('Error Handling and Retry Logic', () => {
    ...
    test('should not retry on non-retryable errors', async () => {
      ...
      mockClaudeService.parseMarketingCommand.mockRejectedValue(nonRetryableError);
  ...
  describe('Health Monitoring', () => {
    test('should provide comprehensive health status', () => {
      ...
      worker.responseCache.set('test-key', { response: {}, timestamp: Date.now() });
      ...
      expect(health.worker.capabilities).toContain('advanced-workflow-patterns');
    ...
    test('should track workflow states', async () => {
      ...
      for (const { input, workflowId } of inputs) {
        await worker.parseMarketingCommandEnhanced(input, { wfRunId: workflowId });
      }
  ...
  describe('Workflow Context Enhancement', () => {
    test('should generate contextual insights from workflow history', async () => {
      ...
      for (const command of commands) {
        await worker.parseMarketingCommandEnhanced(command, { wfRunId: workflowId });
      }
    ...
    test('should generate workflow continuation suggestions', async () => {
      ...
      expect(result.workflowEnhancements.workflowContinuation.suggestedNextSteps).toEqual([
        'validate_audience',
        'generate_content', 
        'schedule_send'
      ]);
      expect(result.workflowEnhancements.workflowContinuation.requiresHumanApproval).toBe(true);
      expect(result.workflowEnhancements.workflowContinuation.canProceedAutomatically).toBe(true);
    });
  });
});
...
describe('Integration Tests', () => {
  ...
  test('should handle complete campaign creation workflow', async () => {
    ...
    const campaignWorkflow = [
      {
        ...
        input: 'Create urgent email campaign for premium customers with 20% discount',
    ...
    expect(contentResult.sagaMetadata.stepName).toBe(campaignWorkflow[1].expected);
  ...
  test('should handle error recovery across workflow steps', async () => {
    ...
    expect(contentResult.sagaMetadata.failureReason).toBe('Content service unavailable');
