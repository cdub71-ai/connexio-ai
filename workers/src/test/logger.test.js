...
jest.mock('winston', () => ({
  ...
  transports: {
    Console: jest.fn().mockImplementation((options) => ({ console: true, ...options })),
    File: jest.fn().mockImplementation((options) => ({ file: true, ...options }))
  }
}));
...
describe('Logger utilities', () => {
  ...
  describe('Logger initialization', () => {
    test('should create logger with correct configuration', async () => {
      ...
      expect(winston.createLogger).toHaveBeenCalledWith({
        ...
        transports: expect.arrayContaining([
          ...
          expect.objectContaining({ file: true, filename: 'logs/error.log', level: 'error' }),
          expect.objectContaining({ file: true, filename: 'logs/combined.log' })
        ]),
        exceptionHandlers: expect.arrayContaining([
          expect.objectContaining({ file: true, filename: 'logs/exceptions.log' })
        ]),
        rejectionHandlers: expect.arrayContaining([
          expect.objectContaining({ file: true, filename: 'logs/rejections.log' })
        ])
      });
    });
    ...
    test('should configure file transports with rotation', async () => {
      ...
      // Error log transport
      const errorTransport = fileTransportCalls.find(call => 
      ...
      const combinedTransport = fileTransportCalls.find(call => 
  ...
  describe('Log format', () => {
    ...
    test('should format minimal log messages', () => {
      ...
      expect(formattedMessage).toBe('2024-01-01 12:00:00.000 [ERROR]: Error message');
    });
  });
